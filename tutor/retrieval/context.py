from tutor.config.rag_settings import answer_rag, question_rag, careless_rag, misconcept_rag, decision_rag, lesson_rag
...
@traceable
def lesson_context(state: TutorState) -> str:

    vectordb = get_vectordb(lesson_rag)
...
@traceable
def decision_context(state: TutorState):

    vectordb = get_vectordb(decision_rag)
    ...
    Correct answer: {correct_answer}
    Learner's answer: {learner_answer}
    The reason for learner's answer: {diagnosis}
    """.strip()
